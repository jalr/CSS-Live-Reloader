...
com_sergiosgc_csslivereloader.reload_css = function() {
	...
	let curentBrowserVersion = Services.appinfo.platformVersion;
	let compareToThisVersion = '43.*';
	if (Services.vc.compare(curentBrowserVersion, compareToThisVersion) == -1) {
		//currentBrowserVersion is less than '43.*' (compareToThisVersion)
		this.cacheService.evictEntries(Ci.nsICache.STORE_ON_DISK);
		this.cacheService.evictEntries(Ci.nsICache.STORE_IN_MEMORY);
	} else {
		//currentBrowserVersion is equal or greater than '43.*' (compareToThisVersion)
